...
class Page_m extends MY_Model{
    ...
    public function get_archive_link(){
        $page = parent::get_by(array('template' => 'template_archive'), TRUE);
    ...
    public function save_order($pages){
        if (count($pages)){
            foreach ($pages as $order=>$page){
                if ($page['item_id'] != ''){
                    $date = array('parent_id' => (int) $page['parent_id'], 'order' => $order);
                    $this->db->set($date)->where($this->_primary_key, $page['item_id'])->update($this->_table_name);
                }
            }
        }
